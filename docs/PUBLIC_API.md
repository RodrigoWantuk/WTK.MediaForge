...
| Configure preview output | `PreviewOutput` / `PreviewWindowOutputSettings` can describe a preview output. | `PreviewOutput`, `MediaForgeOutputs.PreviewWindow`. | `WindowsRenderOutputSinkFactory` creates preview targets only from the hosted-preview adapter path. `BindOutputAsync` with `WinFormsPreviewRenderOutputTarget` remains an advanced/legacy bridge and is not the product path. `AttachSinkAsync` still only accepts `Offscreen`. | Portable lifecycle contract, engine attachment registry, internal Windows render-target bridge, and Studio.Windows `NativeControlHost` creation/attachment are implemented. Preview remains Experimental/proof-gated. | Windows adapter lifecycle tests and hardware reliability evidence are still required before promotion. |
...
| Activate preview output | Preview route consumes completed GPU output leases through a hosted surface. | `MediaForgeEngine.AttachHostedPreviewAsync`, `DetachHostedPreviewAsync`, `HostedPreviewSurface.ResizeAsync`, `RebindAsync`, `DetachAsync`, `CloseAsync`. | A Studio.Windows `NativeControlHost` creates the child HWND internally, owns its `WindowsHostedPreviewSurface`, and attaches the first enabled routed preview output after engine load. The engine owns the attachment registry and routes surface mutations through it. | Implemented integration, Experimental/proof-gated. | Adapter lifecycle tests, Studio visual verification, and hardware hosted-preview reliability evidence remain required before promotion. |
